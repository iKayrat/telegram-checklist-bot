...
### Группа с топиками (forum topics)

Если в группе включены топики и бот должен писать в конкретный топик, а не
в "Общие" — укажите `group_topic_id` в `config.json` (по умолчанию `0` —
General/без топика). Узнать id топика: откройте нужный топик в Telegram,
отправьте туда любое сообщение, затем откройте
`https://api.telegram.org/bot<ТОКЕН>/getUpdates` (как и для `group_chat_id`,
это нужно делать пока бот не запущен через long polling, иначе будет
конфликт) — в объекте сообщения будет поле `"message_thread_id"`, это и
есть нужное число.
