...
// the task id travels as the button's Data payload.
const checkinBtnUnique = "chk"

// BuildChecklistKeyboard renders one button per active task. Any group
// member can press any button — the handler resolves the task from the
// button and the user from the callback sender, so each press only
// affects the presser's own row in the message text.
func BuildChecklistKeyboard(tasks []domain.Task) *telebot.ReplyMarkup {
	markup := &telebot.ReplyMarkup{}
	rows := make([]telebot.Row, 0, len(tasks))
	for i, t := range tasks {
		btn := markup.Data(fmt.Sprintf("%d. %s", i+1, t.Title), checkinBtnUnique, strconv.FormatInt(t.ID, 10))
